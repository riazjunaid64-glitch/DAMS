...
import { uploadProjectMediaBulk, deleteProjectMedia, setProjectCoverMedia } from "../../api/media.ts";
...
export default function ProjectMediaTab({ projectId, media, onMediaChange, user, loading }: Props) {
  ...
  const handleUpload = async (files: File[], uploadDto?: UploadMediaDto) => {
    setUploading(true);
    try {
      const uploaded = await uploadProjectMediaBulk(projectId, files, uploadDto);
    ...
    <div className="py-8 sm:py-10">
      <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div>
            <h2 className="section-title" style={{ marginBottom: 0 }}>Project Media</h2>
