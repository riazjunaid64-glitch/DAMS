...
export default function StaffCashPage({ user }: Props) {
  ...
  return <Container className="py-8">
    ...
    {transfer && selected && <Modal title={`${transfer.id ? "Correct" : "Record"} movement · ${selected.personName}`} close={() => !saving && setTransfer(null)}><div className="space-y-4">
      <label className="block text-sm text-[var(--text-muted)]">Movement<AppSelect value={transfer.type} onChange={(event) => setTransfer({ ...transfer, type: event.target.value as TransferForm["type"] })} className="mt-1 w-full rounded-xl border border-[var(--border)] bg-[var(--input-bg)] p-3 text-[var(--text-primary)]"><option value="FundsGiven">Money given / reimbursement (account → staff)</option><option value="FundsReturned">Cash returned (staff → account)</option></AppSelect></label>
  ...
  {expense && selected && <Modal title={`Record an expense · ${selected.personName}`} close={() => !saving && setExpense(null)}><div className="space-y-4">
    <label className="block text-sm text-[var(--text-muted)]">Expense head<AppSelect value={expense.categoryId} onChange={(event) => setExpense({ ...expense, categoryId: event.target.value })} className="mt-1 w-full rounded-xl border border-[var(--border)] bg-[var(--input-bg)] p-3 text-[var(--text-primary)]"><option value="">Select a head</option>{categories.filter((category) => category.isActive).map((category) => <option key={category.id} value={category.id}>{category.name}{category.isWhtApplicable ? " · WHT" : ""}</option>)}</AppSelect></label>
