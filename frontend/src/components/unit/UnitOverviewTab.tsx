...
const unitStatusColors: Record<string, string> = {
  ...
  pendingreview: "text-blue-400 bg-blue-500/10 border-blue-500/20",
  booked: "text-violet-400 bg-violet-500/10 border-violet-500/20",
};
    ...
    <div className="py-8 sm:py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-5 mb-10">
          ...
          <div className="lg:col-span-2">
            <div className="aspect-[4/3] rounded-2xl border border-[var(--border)] bg-[var(--surface-glass)] overflow-hidden">
              {coverImage ? (
                <img src={resolveMediaUrl(coverImage)} alt={unit.unitNumber} className="h-full w-full object-cover" loading="lazy" decoding="async" />
              ) : (
          ...
          <div className="lg:col-span-3 space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {unit.status.toLowerCase() === "pendingreview" && (
                <div className="flex items-center gap-2 rounded-xl border border-blue-500/20 bg-blue-500/10 px-4 py-2">
                  <span className="h-2 w-2 rounded-full bg-blue-400 animate-pulse" />
                  <span className="text-sm font-medium text-blue-400">Booking Under Review</span>
                </div>
              )}
              {isAdmin && (
                <Button size="sm" variant="outline" onClick={() => setShowEditForm((p) => !p)}>
      ...
      {showBookingModal && (
        <BookingRequestModal
          ...
          onSuccess={() => {
            onUnitUpdate({ ...unit, status: "PendingReview" });
          }}
        />
      )}
    </div>
