...
function AssignmentModal({ category, onClose, onSaved }: { category: DocumentCategory | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Dialog open title={`Assign ${category.name}`} subtitle="Review the target before confirming. Repeating an assignment is safe and does not create duplicates." onClose={() => !saving && onClose()}><form onSubmit={(e) => void submit(e)} className="space-y-4">{error && <FormError text={error} />}<Field label="Apply to"><select className={inputClass} value={mode} onChange={(e) => setMode(e.target.value as AssignmentMode)}><option value="AllActiveCustomers">All existing active customers</option><option value="SelectedCustomers">Selected customers</option><option value="NewCustomersOnly">New customers only</option><option value="None">Do not assign automatically</option></select></Field>{mode === "SelectedCustomers" && <CustomerPicker selected={selected} setSelected={setSelected} />}<Footer saving={saving} onClose={onClose} label="Confirm assignment" /></form></Dialog>;
}
