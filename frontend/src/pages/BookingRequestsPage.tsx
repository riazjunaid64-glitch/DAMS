        ...
        <Container className="relative py-8 sm:py-10">
          ...
          {stats && (
            <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { label: "Pending", value: stats.pending, color: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/20" },
                { label: "Approved", value: stats.approved, color: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
                { label: "Rejected", value: stats.rejected, color: "text-rose-400", bg: "bg-rose-500/10", border: "border-rose-500/20" },
                { label: "Total", value: stats.total, color: "text-[var(--text-heading)]", bg: "bg-[var(--surface-glass)]", border: "border-[var(--border)]" },
              ].map((stat) => (
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                >
                  ...
                  {tab.count !== undefined && (
                    <span className={`rounded-full px-2 py-0.5 text-xs ${
                      activeFilter === tab.id ? "bg-[var(--accent)]/20" : "bg-[var(--surface-glass)]"
                    }`}>
                      {tab.count}
                    </span>
                  )}
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full sm:w-80 rounded-xl border border-[var(--border)] bg-[var(--input-bg)] pl-10 pr-4 py-2.5 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] transition-all focus:border-[var(--accent)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-glow)]"
              />
            </div>
          </div>
        </Container>
              ...
              {requests.map((req) => {
                ...
                return (
                  <div
                    ...
                    className="group cursor-pointer rounded-2xl border border-[var(--border)] bg-[var(--surface-glass)] p-5 transition-all hover:border-[var(--border-hover)] hover:shadow-lg hover:bg-[var(--surface-glass-hover)]"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-semibold text-[var(--text-heading)] group-hover:text-[var(--accent)] transition-colors">
                          {req.fullName}
                        </p>
                        <p className="text-xs text-[var(--text-muted)]">{req.email}</p>
                      </div>
                      <span className={`rounded-full border px-2.5 py-0.5 text-[10px] font-semibold ${status.color}`}>
                        {status.label}
                      </span>
                    </div>

                    <div className="rounded-xl bg-[var(--bg-primary)] border border-[var(--border)] p-3 mb-3">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-sm text-[var(--text-primary)]">{req.unitNumber}</span>
                        <span className="rounded bg-[var(--accent-glow)] px-1.5 py-0.5 text-[9px] font-medium text-[var(--accent)] uppercase">
                          {req.unitType}
                        </span>
                      </div>
                      <p className="text-xs text-[var(--text-muted)]">{req.projectName}</p>
                      <p className="text-sm font-semibold text-[var(--text-heading)] mt-1">
