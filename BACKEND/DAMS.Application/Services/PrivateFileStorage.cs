...
{
    /// <summary>
    /// Stores private files on disk outside the web root under opaque generated names.
    /// Keys are single file names — anything that could escape the root is rejected before
    /// it reaches the file system.
    /// </summary>
    public abstract class PrivateFileStorage
    {
        private readonly string _storageRoot;

        protected PrivateFileStorage(string storageRoot)
        {
            if (string.IsNullOrWhiteSpace(storageRoot))
                throw new ArgumentException("A private storage path is required.", nameof(storageRoot));
        ...
        public Task<Stream?> OpenReadAsync(string storedFileName, CancellationToken cancellationToken = default)
        {
            ...
            Stream? stream = File.Exists(fullPath)
                ? new FileStream(fullPath, FileMode.Open, FileAccess.Read, FileShare.Read | FileShare.Delete, 81920,
